...
import type { RequisitionSheet, RequisitionItem, AdminSettings } from "@shared/schema";
...
export default function RequisitionSheetPage() {
  ...
  useEffect(() => {
    ...
    if (storedSettings) {
      const settings: AdminSettings = JSON.parse(storedSettings);
      setHeaderSettings({
        title: settings.requisitionHeaderTitle || "Daily Our Bangladesh",
        subtitle: settings.requisitionHeaderSubtitle || "Requisition Month of",
        preparedBy: settings.requisitionPreparedBy || "Administrator",
      });
    }
  }, []);
      ...
      <div className="w-full px-6 py-6 flex-1">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-semibold text-foreground">{headerSettings.title}</h1>
              <h3 className="text-lg font-medium text-foreground mt-1">{headerSettings.subtitle} {monthName}</h3>
              <p className="text-sm text-muted-foreground mt-1">
                Prepared by: {headerSettings.preparedBy}
              </p>
            </div>

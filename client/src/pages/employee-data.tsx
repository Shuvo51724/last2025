...
import { Employee, EmployeeUser } from "@shared/schema";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Plus, Download, Search, Filter, Copy, CheckCircle } from "lucide-react";
...
import { createEmployeeUser, generateEmployeePassword, deleteEmployeeUser } from "@/lib/employeeUserService";
...
export default function EmployeeData() {
  ...
  const designations = useMemo(() => {
    const unique = new Set(employees.map(e => e.designation).filter((d): d is string => Boolean(d)));
    return Array.from(unique);
  }, [employees]);

  const shifts = useMemo(() => {
    const unique = new Set(employees.map(e => e.officeShift).filter((s): s is string => Boolean(s)));
    return Array.from(unique);
  }, [employees]);
  ...
  const handleUpdateEmployee = (id: string, updates: Partial<Employee>) => {
    ...
    const updatedEmployee = updated.find(emp => emp.id === id);
    if (updatedEmployee && updatedEmployee.name && updatedEmployee.employeeId) {
      const existingUser = localStorage.getItem("dob_employee_users");
      const users: EmployeeUser[] = existingUser ? JSON.parse(existingUser) : [];
      const userExists = users.some(u => u.employeeId === updatedEmployee.employeeId);
      
      if (!userExists) {
        const password = generateEmployeePassword();
        const userId = updatedEmployee.employeeId;
        ...
        createEmployeeUser(employeeUser);
        
        // Sync with moderators list in admin panel
        const storedModerators = localStorage.getItem("dob_moderators");
        const moderators = storedModerators ? JSON.parse(storedModerators) : [];
        
        const modExists = moderators.some((m: any) => m.userId === userId);
        if (!modExists) {
          const newModerator = {
            id: crypto.randomUUID(),
            name: updatedEmployee.name,
          ...
          localStorage.setItem("dob_moderators", JSON.stringify(moderators));
        }
        ...
        toast({
          ...
          description: `Login credentials generated for ${updatedEmployee.name}`,
        });
      }
    }
  };
  ...
  const handleDeleteEmployee = (id: string) => {
    ...
    if (employee && employee.employeeId) {
      deleteEmployeeUser(employee.employeeId);
    }
    ...
    toast({
      ...
      description: "Employee record and login credentials removed",
    });
  };
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Employee Login Credentials Generated
            </DialogTitle>
            <DialogDescription>
              Save these credentials securely. The employee can use these to login.
            </DialogDescription>
          </DialogHeader>
